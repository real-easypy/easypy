...
class TeamsLock(object):
    """
    The lock always belong to one "team"(or no team), and other team members
    can join the lock - but not members of other teams!

    * Initialize with a list of team names
    * Use the `.acquire_for_team(<team-name>)` context manager to lock for your team

    Example:

        lock = TeamsLock('foo', 'bar')

        with lock.acquire_for_team('foo'):

            # Will enter - same team
            with lock.acquire_for_team('foo'):
                pass

            # Will block - different team
            with lock.acquire_for_team('bar'):
                pass
    """
